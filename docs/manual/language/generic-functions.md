...
The running example here is `tests/nod-od-suite/fixtures/area-shapes.dylan`, which
defines a `<shape>` hierarchy and a `area` GF with two methods. Slot accessor generics
follow the same pattern: `define class <circle>` with `slot radius` causes sema to
auto-generate a method on the generic `radius` specialised to `<circle>` (see
`src/nod-sema/src/lower.rs`, Phase 3). The stdlib adds further GF examples:
`size`, `element`, `element-setter`, `remove-key!`, `keys`, `values`, and
`object-hash` all have methods specialised to concrete classes such as `<table>`
...
*generic function*. A generic is a process-global object (`GenericFunction` in
`src/nod-runtime/src/dispatch.rs:172`) that owns a `Vec<Method>` under an
`RwLock`. Any library that has access to the generic can add a method to it —
...
The runtime test in `tests/nod-tests/tests/dispatch.rs:151` exercises this directly:
a generic `disp-intersect` has three methods registered with two-argument specialiser
vectors:
...
position `i` (`src/nod-runtime/src/dispatch.rs:486-494`).
...
(`src/nod-runtime/src/dispatch.rs:496-514`).
...
the next-most-specific method in the sorted chain. The runtime pushes a
`MethodChainFrame` onto a thread-local stack before calling the head method; the Dylan
form `next-method()` lowers to a call to `nod_next_method`, which pops and invokes the
next body (`src/nod-runtime/src/dispatch.rs:948`, `src/nod-sema/src/lower.rs:5271`).
`next-method?()` tests whether a next method exists without invoking it. For the
`SealedDirectCall` path the fallback chain is pre-computed at compile time (see
...
The algorithm (`src/nod-sema/src/c3.rs:57`, signature
...
4. If no good head can be found, the inheritance graph has conflicting orderings
   and `C3Error::InconsistentMerge` is returned (surfaced as
   `LoweringError::InconsistentInheritance`).

The diamond test `T4-diamond` in `tests/nod-tests/fixtures/dylan-c3-smoke.dylan:363-372`
runs this case in Dylan: both `<b>` and `<c>` declare `<a>` as their direct super;
...
The CPL computed by `c3_linearise` is stored in `ClassMetadata` and used both for
...
test `T6-cycle` in the fixture: one parent CPL is `[<p1>, <x>, <y>]` and the other is
`[<p2>, <y>, <x>]` — C3 cannot pick a consistent head and returns an error. The
compiler rejects the `define class` form.

## Sealing makes dispatch static

By default a generic function is *open*: new methods can be added at any time and
dispatch is resolved at runtime via an inline cache (`CacheSlot` in
`src/nod-runtime/src/dispatch.rs:139`).
...
sealing facts plus the type estimates from `narrow_function` allow the compiler to
...
When sealing succeeds, the compiler records a `DispatchResolution` in the
`LoweredModule` (visible via `--dump-dispatch`). When in doubt — ambiguous methods,
open generic, unknown generic — the node stays as `Dispatch` and the runtime inline
cache handles it safely (`src/nod-sema/src/optimise/dispatch.rs:24`, the soundness
rule: "when in doubt, leave as Dispatch").

See [Sealing](sealing.md) for the language-programmer's contract and
[Semantic Analysis](../compiler/sema.md) for the dispatch planning passes
(`narrow_function` + `resolve_dispatches`).

## How it is implemented

This page covers the language surface. The implementation splits across two crates:

**Front end (`nod-sema`):** Phase 2 registers generic names; Phase 4 lowers
`define method` bodies to DFM and produces `MethodRegistration` records
(specialiser vector + body symbol). After JIT compilation, `register_methods`
calls `nod_runtime::add_method_named` to bind each JIT'd function pointer into
...
**Back end (`nod-runtime`):** `nod_dispatch` is the runtime entry point for
unresolved `Dispatch` nodes. It checks the per-call-site `CacheSlot`, falls through
to `lookup_applicable_methods` on a cache miss, pushes a `MethodChainFrame` for
`next-method`, writes the result back to the cache slot, and calls the method body.
The generation counter on `GenericFunction` is the only stale-cache signal; cache
reads use `Relaxed` atomics.
See [Runtime & Object Model](../compiler/runtime.md).

---
[Manual home](../index.md) · [Macros](macros.md)
