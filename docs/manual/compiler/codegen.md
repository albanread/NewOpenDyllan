...
> Crate: `src/nod-llvm`  ·  Status: live — permanent Rust + LLVM back-end
...
The DFM hexagon is the cut line: everything above migrates to Dylan; everything
below is permanent Rust + LLVM. `codegen.rs` sits immediately below the cut.
...
their home allocas rather than from the original `get_nth_param` SSA value — this is
the GAP-011 fix (`codegen.rs:2356`).
...
values at jump-emit time (not end-of-function) to avoid GAP-007: a safepoint reload
after the jump would rebind `temps[t]` to a different SSA value (`codegen.rs:2388`).
...
(`codegen.rs:3681`). Special-case dispatch for `%pair-*` list builtins, Sprint 19
condition builtins, Sprint 20b collection/FIP primitives, and Win64 FFI trampolines
(`codegen.rs:3607`).
...
3. If the root is a function parameter, also write the reloaded value back to the
   ...
   address (GAP-011 invariant B, `codegen.rs:4619`).
...
bitcode portable across process restarts (Sprint 38, `codegen.rs:766`). The
...
GVN passes. The design note "emit good IR for LLVM" from `docs/ARCHITECTURE.md` is
the guiding principle.

## Invariants & gotchas

- **All allocas must be in the entry block.** An alloca inside a loop body leaks
  stack at `-O0` (GAP-010). `build_entry_alloca` (`codegen.rs:4652`) positions the
...
- **Jump args are resolved at jump-emit time.** Deferring TempId-to-SSA resolution
  to end-of-function would cause safepoint reloads to corrupt phi incomings (GAP-007,
  `codegen.rs:2160`).
