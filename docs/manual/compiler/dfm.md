...
splits the NewOpenDylan compiler in two: every front-end phase lowers to it;
every back-end phase consumes it. It is the permanent contract between them.

> Crate: `src/nod-dfm`  ·  Status: live — the front-end/back-end boundary
...
The hexagon marks the architectural boundary. A Rust-emitted DFM module and
a Dylan-emitted DFM module are **the same data structure with the same
semantics** — the back-end cannot tell them apart. That property is what makes
the front-end migration safe: each phase can be swapped to Dylan and validated
against the Rust phase by comparing DFM output, with the back-end held
constant. See [self-hosting](self-hosting.md) and
...
the call with `nod_register_root` / `nod_unregister_root` pairs for each root,
reloading after the call returns so GC-driven evacuations are observed.
...
The earlier per-block approximation was unsound for temps live *through* a
block without being mentioned there. The current global fixpoint closes that
gap (GAP-011, `liveness.rs:1`).
...
dominance. Full dominance enforcement is deferred (`verify.rs:70`).
...
format bumps the cache version.
...
- **`TypeEstimate` is an estimate, not a proof.** `Top` is always safe; the
  ...
  protection even if they hold a fixnum). Sprint 11c's `gc.statepoint` upgrade
  tightens this.
- **`Singleton(u64)` is carried but not populated.** The Sprint 15 narrower
...
- [Self-hosting](self-hosting.md) — how the front-end migrates to Dylan while DFM stays constant
- [`docs/ARCHITECTURE.md`](../../ARCHITECTURE.md) — the canonical front-end/back-end split

---
[Manual home](../index.md) · [Compiler overview](overview.md) · [Sema](sema.md) · [Codegen](codegen.md)
