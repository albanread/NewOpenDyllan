...
> Crate: `src/nod-runtime`  ·  Status: live — permanent Rust back-end
...
LLVM-emitted code; the slow path runs Rust.
...
Sprint 20 established the abstract collection hierarchy and the forward
iteration protocol (FIP). All are registered as user classes at boot time
via `ensure_registered` (`collections.rs:94`).
...
(`tables.rs:64-67`). Sprint 22 hash methods: `<integer>` — multiplicative
mix hash; `<byte-string>` and `<symbol>` — FNV-1a 64-bit over the byte payload.
...
- **`is_no_alloc` bypasses GC root bracketing, not the liveness pass.** A
  ...
  only the codegen-emitted `nod_register_root`/`nod_unregister_root` brackets
  are skipped (`dfm.md`).
...
---
[Manual home](../index.md) · [Compiler overview](overview.md) · [DFM](dfm.md) · [Codegen](codegen.md) · [GC](gc.md)
