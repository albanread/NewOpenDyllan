...
top-level definitions. This page describes the syntax as NewOpenDylan parses it.
For the internals — how the lexer, fragment builder, and parser are implemented —
see [Reader: lexer & parser](../compiler/reader.md).
...
The lexer emits 59 token kinds. The taxonomy below groups them by category.
...
Only `define`, `end`, and `otherwise` are reserved at the lexer level. The
parser classifies all other contextual words — `if`, `let`, `class`, `method`,
`for`, `while`, `sealed`, `open`, `abstract`, `library`, `module`, `begin`,
`block`, `case`, `select`, `when`, `unless`, `signal`, `local`, `cleanup`,
`exception` — by matching the text of an `Ident` token at the appropriate parse
site.
...
A standalone `<` after whitespace enters operator mode.

Operators used as value names are written with a leading backslash: `\+`, `\<=`,
`\~==`. These lex as `EscapedIdent`. The backslash is consumed; the token's text
...
`init-value:`, `setter:`, `allocation:`). See [Types & classes](types-and-classes.md).
...
`define constant` and `define variable` share the same surface shape — a name,
an optional type annotation, and an initialiser.
...
captures it as raw fragments. The macro expander processes it later. See
[Macros](macros.md).

### define library and define module

These form the namespace graph; they hold `use` / `export` / `create` clauses.
See [Modules & libraries](modules-and-libraries.md).
...
Dylan code. See [FFI](../compiler/ffi.md).
...
These are hardcoded in the parser and produce structured AST nodes:
...
These are also hardcoded and appear at statement position:
...
clause. See [Conditions](conditions.md).

### Macro-defined forms

Forms like `when`, `unless`, `cond`, and `for-each` are **not** hardcoded in
the parser. They are defined as `define macro` in the stdlib and expand to
kernel forms at macro-expansion time. See [Macros](macros.md) for the expansion
model.
...
`select` (the dispatching switch form) is **not yet implemented** in the
parser; the parser emits a structured diagnostic instead of an AST node. `case`
is fully implemented. Use `case` for now, or define a `select`-shaped macro.
...
The multi-binder `let` binds several names at once. The RHS is evaluated once
and the binders receive distinct return values. This is a kernel form
(`Statement::Let` with multiple binders), not a macro.
...
**The binary operators recognised** (from the `BinOp` enum):
`+`, `-`, `*`, `/`, `^` (power), `mod`, `rem`, `=`, `==`, `~=`, `~==`, `<`,
`>`, `<=`, `>=`, `&`, `|`, `:=`.

**`Precedence: c` header pragma.** A file may opt into conventional C-style
precedence by placing `Precedence: c` in its preamble. The stdlib uses this
pragma. It is a migration bridge, not a language feature; the DRM flat rule is
the canonical Dylan behaviour.
...
---
Next: [Types & classes](types-and-classes.md) · See also [Reader: lexer & parser](../compiler/reader.md)
