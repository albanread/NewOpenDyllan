...
The `area-shapes` fixture is the canonical single-dispatch demo. It defines
two shape classes, declares a generic function, and gives each shape its own
method:
...
grouping with parentheses is required for the sum-of-squares shape.

## The built-in class tower

Every Dylan value is an instance of `<object>`. The seed classes registered
by the implementation at startup form two groups: scalar immediates and
structured heap objects. The condition and collection hierarchies extend this
at runtime.
...
Concrete scalar types (`<integer>`, `<boolean>`, `<symbol>`, etc.) are
...
registered as user classes at startup by the runtime. They are not shown above
to keep the diagram readable. See [Conditions](conditions.md).

User-defined classes (`define class <foo> (<bar>)`) extend the hierarchy from
any of these roots. The compiler assigns each a unique class id; the GC knows
their slot layouts; and dispatch walks the class precedence list (C3 order) to
find the most-specific applicable method.
...
NewOpenDylan is a self-hosting Dylan compiler whose front end (lexer, parser,
macro expander, semantic analysis, AST-to-DFM lowering) is written in Dylan
and whose back end (DFM optimiser, LLVM codegen, JIT and AOT, GC runtime, FFI)
is written in Rust and LLVM. The two halves meet at the DFM IR.

**What runs today:**

- The compiler JITs non-trivial Dylan programs via the driver's `eval`
  subcommand and the REPL, and AOT-compiles standalone Win64 EXEs via the
  driver's `build` subcommand.
...
- `nod-ide.exe` — a graphical source editor with DirectWrite rendering, a rope
  buffer, and syntax colouring — is itself written entirely in Dylan and
  AOT-built by the compiler.
- The front end — lexer, parser, macro expander, sema, and AST → DFM
  lowering — is written in Dylan and compiled into the driver.
- The stdlib macro surface includes `unless`, `when`, `for-each`,
  `with-cleanup`, and `cond` — all `define macro` forms, not hardcoded AST
  nodes.
...
- The GC's alloca-tracker verifier is not yet built. The precise-root
  invariant holds by construction; a future codegen change could regress it
  silently.
...
- Cross-module symbol linkage for separately compiled libraries is not yet
  implemented.
- The DRM condition hierarchy uses multiple inheritance; the seed classes use
  single inheritance as a pragmatic shortcut today.

See [Known limitations](../reference/known-limitations.md) for the full list.

---
Next: [Syntax & lexical structure](syntax.md) · See also [Architecture](../architecture.md)
